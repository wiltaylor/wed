...
pub fn buffer_top(_buf: &Buffer, _cur: Cursor) -> Cursor {
    Cursor::new(0, 0)
}
...
fn iter_backward(buf: &Buffer, start: Cursor) -> Vec<(usize, usize, char)> {
    ...
    for row in (0..=start.row).rev() {
        let line = buf.line(row).to_string();
        let chars: Vec<char> = line.chars().collect();
        let end = if row == start.row {
            (start.col + 1).min(chars.len())
        } else {
            chars.len()
        };
...
pub fn word_backward(buf: &Buffer, cur: Cursor, n: usize) -> Cursor {
    ...
    for _ in 0..n {
        ...
        let mut i = 1;
        while i < seq.len()
            && (classify(seq[i].2) == CharClass::Space || classify(seq[i].2) == CharClass::Newline)
        {
            i += 1;
        }
        if i >= seq.len() {
...
pub fn word_forward_big(buf: &Buffer, cur: Cursor, n: usize) -> Cursor {
    ...
    for _ in 0..n {
        ...
        // walk to whitespace
        while i + 1 < seq.len()
            && classify(seq[i].2) != CharClass::Space
            && classify(seq[i].2) != CharClass::Newline
        {
            i += 1;
        }
...
pub fn word_backward_big(buf: &Buffer, cur: Cursor, n: usize) -> Cursor {
    ...
    for _ in 0..n {
        ...
        while i + 1 < seq.len()
            && classify(seq[i + 1].2) != CharClass::Space
            && classify(seq[i + 1].2) != CharClass::Newline
        {
            i += 1;
        }
        if i >= seq.len() {
...
pub fn match_bracket(buf: &Buffer, cur: Cursor) -> Cursor {
    ...
    for &(o, c) in pairs {
        if here == o {
            return scan_match(buf, cur, o, c, true);
        }
        if here == c {
            return scan_match(buf, cur, c, o, false);
        }
    }
    cur
}
...
fn scan_match(buf: &Buffer, start: Cursor, open: char, close: char, forward: bool) -> Cursor {
    let mut depth = 0i32;
    let seq = if forward {
        iter_forward(buf, start)
    } else {
        iter_backward(buf, start)
    };
    for (r, c, ch) in seq {
        if ch == open {
            depth += 1;
...
pub fn paragraph_forward(buf: &Buffer, cur: Cursor, n: usize) -> Cursor {
    ...
    for _ in 0..n {
        let mut r = row + 1;
        // skip blank lines
        while r < buf.line_count() && buf.line_len_chars(r) == 0 {
            r += 1;
        }
        // walk to next blank
        while r < buf.line_count() && buf.line_len_chars(r) > 0 {
...
pub fn paragraph_backward(buf: &Buffer, cur: Cursor, n: usize) -> Cursor {
    ...
    for _ in 0..n {
        ...
        let mut r = row - 1;
        while r > 0 && buf.line_len_chars(r) == 0 {
            r -= 1;
        }
        while r > 0 && buf.line_len_chars(r) > 0 {
            r -= 1;
